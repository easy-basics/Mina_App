...
- 底部「去下单」→ 选择板布/大货 → 弹层（`OrderSheet`）选门店、颜色、数量
  - **板布**：底部「加入购物车」「立即购买 ¥合计」；立即购买跳转确认订单页
  - **大货**：底部「大货下单（面议）」；直接创建订单并跳转订单详情，不进入确认订单页

### 5.3 购物车（`pages/cart/index`）

- 列表按 **布版** / **大货** 分组展示
- 修改数量、删除行
- 分别「布版结算」「大货结算」，跳转确认订单页（`pages/order/checkout`）

### 5.4 结算与下单（`pages/order/checkout`）

**板布（商品详情立即购买 / 购物车结算）**

- 确认订单页：配送方式（自提展示门店地址 / 邮寄选收货地址）、商品明细、订单备注
- 底部合计 +「提交订单」→ 创建订单 → 唤起微信支付（开发环境 mock）
- 支付成功或未完成均可查看订单详情；未完成可在详情继续支付

**大货（仅购物车结算经确认订单页）**

- 确认订单页：选择门店、商品明细（面议）、订单备注
- 提交后状态 `submitted`，支付状态 `offline`，无在线支付

**大货（商品详情弹层）**

- 弹层内直接提交，无配送/备注步骤

下单成功后清除对应购物车 SKU（购物车来源时）。
